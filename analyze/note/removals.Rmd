...
rem_hist <- read_delim(here::here("analyze", "input", "ice_removals_fy12-23ytd.csv.gz"), delim='|') %>% 
...
dat_new <- removal_df %>% 
  ...
  filter(departed_date >= "2023-10-01") %>% 
  group_by(docket_aor, fy) %>% 
  summarize(n = n()) %>% 
  rename(aor = docket_aor) %>% 
  mutate(dataset = "DDP")

dat <- rbind(dat_hist, dat_new) %>% 
  mutate(fy = as.numeric(fy))

p1 <- dat %>% 
  filter(aor %in% c("SEA", "Seattle Area of Responsibility")) %>% 
  ggplot(aes(x = fy, y = n, fill = dataset)) +
  geom_col() +
  labs(title = "Removals from ICE Seattle AOR",
       subtitle = "Oct. 2011 - Jul. 2025",
       caption = "Source: ICE data obtained via FOIA")
...
p2 <- dat %>% 
  ggplot(aes(x = fy, y = n, fill = dataset)) +
  geom_col() +
  # gghighlight(aor %in% c("ELP", "El Paso Area of Responsibility")) +
  labs(title = "Annual Nationwide ICE Removals",
       subtitle = "Oct. 2011 - Jul. 2025",
       caption = "Source: ICE data obtained via FOIA")
