...
# trac_facil <- read_delim(here::here('analyze', 'input', 'trac_foia_24-05655_facils.csv'), delim='|')
# 
# df <- df %>% 
#   mutate(trac_facil = detention_facility_code %in% unique(trac_facil$detention_facility_code))
# 
# # Most detention records involve known facilities (TRAC FOIA)
# sum(df$trac_facil) / nrow(df)
# 
# # Why are these facilities not represented in TRAC data?
# unknown_facil <- unique(df[df$trac_facil == FALSE, c("detention_facility_code", "detention_facility")]) %>% 
#   arrange(detention_facility)
# 
# write_delim(unknown_facil, "ddp_facil_not_in_trac.csv", delim=',')
...
select_facil <- c("FDLHOLD", "SEAHOLD", "YAKHOLD", "SPOHOLD", "POOHOLD", "EUGHOLD", "MEDHOLD")
