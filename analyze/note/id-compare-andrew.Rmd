...
```{r}
joined_df <- inner_join(detainer_df, arrest_df, by="unique_identifier", na_matches="never")
joined_df<- joined_df%>% 
  mutate(detainer_date_diff = difftime(detainer_prepare_date, apprehension_date.y, units = "days"))

filtered_joined_df <- filter(joined_df, detainer_date_diff < 2 & facility_state == "WASHINGTON" & !(detention_facility %in% c("SEATAC FED.DET.CENTER", "STATE PRISON, SHELTON")))
filtered_joined_df[c("detainer_prepare_date", "apprehension_date.y", "unique_identifier", "detention_facility")]
```
```{r}
ggplot(filtered_joined_df, mapping=aes(x=detention_facility)) +
  geom_bar() +
