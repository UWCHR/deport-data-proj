...
general_caption <- "Data: ICE arrests obtained by Deportation Data Project\nAnalysis: University of Washington Center for Human Rights"
...
figure_2 <- dat_2 |>
  ...
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by Criminal Conviction Status", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
...
figure_3 <- dat_2 |>
  ...
  coord_cartesian(clip = "off") +
  labs(fill = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by Criminal Conviction Status", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_2 <-
  ...
  scale_color_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by Criminal Conviction Status (Seattle AOR)", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_3 <- dat %>% 
  ggplot(aes(week, n, fill = criminality, group = criminality)) +
  geom_vline(data = NULL, xintercept = as.Date("2025-01-20"), color = gray(0.2), linetype = "dashed") +
  annotate("text", x = as.Date("2025-01-20") - days(7), y = max(dat$n)*.75, label = "Trump II\ninauguration", hjust = 1, size = 3.5, color = gray(0.2)) +
  geom_area(linewidth = 0.8) +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
...
dat <- sea %>% 
  ...
  filter(month <= "2025-09-01") %>% 
  # mutate(criminality = factor(criminality, levels = c(TRUE, FALSE), labels = c("Criminal conviction", "No criminal conviction"))) %>% 
  count(apprehension_criminality, month)

figure_4 <- dat %>% 
  ggplot(aes(month, n, fill = apprehension_criminality, group = apprehension_criminality)) +
  geom_vline(data = NULL, xintercept = as.Date("2025-01-20"), color = gray(0.2), linetype = "dashed") +
  annotate("text", x = as.Date("2025-01-20") - days(7), y = max(dat$n)*1.25, label = "Trump II\ninauguration", hjust = 1, size = 3.5, color = gray(0.2)) +
  geom_area(linewidth = 0.8) +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_viridis_d() +
  coord_cartesian(clip = "off") +
  labs(fill = "", title = "Monthly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by Criminal Conviction Status", x = "", y = "Number of arrests per Month", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
dat_out <- sea %>% 
  ...
  filter(month <= "2025-09-01") %>% 
  # mutate(criminality = factor(criminality, levels = c(TRUE, FALSE), labels = c("Criminal conviction", "No criminal conviction"))) %>% 
  count(apprehension_criminality, month, apprehension_state)

write_delim(dat_out, "sea_crim_state_month.csv", delim=",")
...
pre_post_mean <- sea |>
  ...
  mutate(month = floor_date(apprehension_date, unit = "month"),
         trump = apprehension_date >= "2025-01-20") %>% 
  count(month, trump) %>% 
  group_by(trump) %>% 
  summarize(arrests_per_month = mean(n))

figure_3 <-
  sea |>
    filter(apprehension_date >= "2023-09-01",
         apprehension_date <= "2025-09-30"
         ) |>
  mutate(month = floor_date(apprehension_date, unit = "month")) |>
  count(month) |>
  ggplot(aes(month, n)) +
geom_vline(data = NULL, xintercept = as.Date("2025-01-20"), color = gray(0.2), linetype = "dashed") +
  annotate("text", x = as.Date("2025-01-20") - days(7), y = 300, label = "Trump II\ninauguration", hjust = 1, size = 3.5, color = gray(0.2)) +
  geom_line(linewidth = 0.8, color = "#440154FF") +
  scale_color_viridis_d() +
  scale_y_continuous(labels = scales::comma) +
  coord_cartesian(clip = "off") +
  ylim(0, NA) +
  labs(title = "Monthly arrests by Immigration and Customs Enforcement (ICE)",
       subtitle = "Seattle Area of Responsibility",
       x = "", y = "Number of arrests per month", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,10,0)),
        axis.title.x = element_blank(),
...
figure_2 <- dat_2 %>% 
  ...
  scale_color_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by Criminal Conviction Status (WA state)", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_2 <- dat_2 %>% 
  ...
  scale_color_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by Criminal Conviction Status (OR state)", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_1 <-
  ...
  scale_color_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by whether associated with detention", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_2 <-
  ...
  scale_color_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by whether associated with detention (SEA AOR)", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_3 <-
  arrest_df |>
  filter(apprehension_aor == "Seattle Area of Responsibility",
         apprehension_date >= "2023-09-01",
         apprehension_date <= "2025-09-30") %>% 
  mutate(month = floor_date(apprehension_date, "month")) |>
  mutate(id_in_det = factor(id_in_det, levels = c(TRUE, FALSE), labels = c("arrest with detention", "arrest with no detention"))) |>
  count(id_in_det, month) |>
  ggplot(aes(month, n, color = id_in_det, group = id_in_det)) +
  geom_vline(data = NULL, xintercept = as.Date("2025-01-20"), color = gray(0.2), linetype = "dashed") +
  annotate("text", x = as.Date("2025-01-20") - days(7), y = 300, label = "Trump II\ninauguration", hjust = 1, size = 3.5, color = gray(0.2)) +
  geom_line(linewidth = 0.8) +
  scale_y_continuous(labels = scales::comma) +
  ...
  coord_cartesian(clip = "off") +
  ylim(0, NA) +
  labs(color = "", title = "Monthly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by whether associated with detention (SEA AOR)", x = "", y = "Number of arrests per month", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_1 <- dat_natl |>
  ...
  scale_color_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by whether associated with detention (national)", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_3 <- dat_wa |>
  ...
  scale_color_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by whether associated with detention (Washington state)", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
figure_4 <- dat_or |>
  ...
  scale_color_manual(values = c("#0072B2", "#D55E00")) +
  coord_cartesian(clip = "off") +
  labs(color = "", title = "Weekly arrests by Immigration and Customs Enforcement (ICE)", subtitle = "by whether associated with detention (Oregon state)", x = "", y = "Number of arrests per Week", caption = general_caption) +
  theme_minimal() +
  theme(plot.margin = margin(5, 0, 5, 5),
        plot.title = element_text(margin = margin(0,0,5,0)),
        plot.subtitle = element_text(margin = margin(0, 0, 10, 0)),
...
dat <- arrest_df %>% 
  filter(str_detect(apprehension_site_landmark, "YAKIMA")) %>% 
  mutate(week = floor_date(apprehension_date, "week", week_start = "Monday")) %>% 
  count(week, apprehension_site_landmark)

p1 <- dat %>% 
  ggplot(aes(x=week, y=n, fill=apprehension_site_landmark)) +
  geom_col() +
  labs(title = "Weekly total ICE arrests mentioning `Yakima`")
...
p1 <- dat %>% 
  ...
  geom_col() +
  scale_fill_viridis_d() +
  labs(title = "Seattle AOR arrests resulting in detention",
...
dat_2 <- detention_df %>% 
  ...
  mutate(detention_facility = case_when(detention_facility %in% head(top_facil$detention_facility, 6) ~ detention_facility,
         TRUE ~ "ALL OTHERS")) %>% 
  count(trump, detention_facility, final_program)

p1 <- dat_2 %>% 
  ggplot(aes(y = detention_facility, x = n, fill = final_program)) +
  geom_col() +
  facet_wrap(~trump, nrow = 2)
