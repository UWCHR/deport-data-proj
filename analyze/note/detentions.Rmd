...
dat <- df %>% 
  ...
  dplyr::select(unique_identifier, detention_facility_code, placement_count, stay_count) %>% 
  mutate(unique_identifier = paste(unique_identifier, stay_count, sep = '-'))

dat_wide <- dat %>% 
  pivot_wider(id_cols = c(unique_identifier), names_from = placement_count, names_prefix = "facil_", values_from = detention_facility_code)
