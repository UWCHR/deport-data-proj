...
# Apply function to timeline
example_headcount <- lapply(timeline, headcounter, df=temp_df, group_vars=c('prev_aor'))
...
p1 <- example_headcount_data %>% 
  filter(date >= "2023-11-15") %>% 
  ggplot(aes(x = date, y = n, color = prev_aor), group = prev_aor ) +
  geom_line() +
  labs(title = "Daily detained population by `prev_aor`",
       subtitle = "Northwest ICE Processing Center")
