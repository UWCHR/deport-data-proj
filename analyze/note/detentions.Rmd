...
dat <- df %>% 
  filter(!is.na(detention_release_reason)) %>% 
  mutate(week = floor_date(book_in_date_time, "week", week_start = "Monday"),
         trump = book_in_date_time >= "2025-01-20") %>% 
  count(trump, week, detention_release_reason) %>% 
  group_by(trump, detention_release_reason) %>% 
  summarize(weekly_mean_releases = mean(n))

p1 <- dat %>% 
  ggplot(aes(y = detention_release_reason, x = weekly_mean_releases, fill = trump)) +
  geom_col(position='dodge')

p1

dat2 <- dat %>% 
  pivot_wider(id_cols='detention_release_reason', names_from = "trump", names_prefix="trump_", values_from= 'weekly_mean_releases') %>% 
  mutate(pct_change = ((trump_TRUE - trump_FALSE) / trump_FALSE) * 100) %>% 
  arrange(desc(pct_change))

p2 <- dat2 %>% 
  filter(!is.na(pct_change)) %>% 
  mutate(detention_release_reason = fct_reorder(as.character(detention_release_reason), pct_change),
         pos = pct_change > 0) %>% 
  ggplot(aes(x = pct_change, y = detention_release_reason, fill = pos)) +
  geom_col(show.legend = FALSE) +
  labs(title = "% change in weekly avg. detention release in first weeks of Trump II")
