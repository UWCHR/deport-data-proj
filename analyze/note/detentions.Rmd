...
temp_df <- df %>% 
  ...
  mutate(first_placement = placement_count == 1)

# Apply function to timeline
example_headcount <- lapply(timeline, headcounter, df=temp_df, group_vars=c('first_placement'))
...
p1 <- example_headcount_data %>% 
  ...
  ggplot(aes(x = date, y = n, color = first_placement), group = first_placement ) +
  geom_line() +
  labs(title = "Daily detained population by `first_placement`",
...
# Apply function to timeline
example_headcount <- lapply(timeline, headcounter, df=temp_df, group_vars=c('final_program_group'))
...
p1 <- example_headcount_data %>% 
  filter(date >= "2023-11-15") %>% 
  ggplot(aes(x = date, y = n, fill = final_program_group), group = final_program_group ) +
  geom_area() +
  labs(title = "Daily detained population by `final_program_group`",
       subtitle = "Northwest ICE Processing Center")
